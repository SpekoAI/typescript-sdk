...
if (!apiKey) {
  console.error('Missing SPEKO_API_KEY. Generate one at https://platform.speko.dev/agents/keys');
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    console.warn(
      `(no sample.wav at ${audioPath} — drop a short WAV clip there to test transcribe)`,
    );
