  ...
  private dispatchIncoming(data: unknown): void {
    ...
    switch (t) {
      ...
      case 'ready':
        this.emit({
          type: 'ready',
          inputSampleRate: Number(parsed['inputSampleRate'] ?? this.inputSampleRate) as
            | 16000
            | 24000,
          outputSampleRate: Number(parsed['outputSampleRate'] ?? this.outputSampleRate) as
            | 16000
            | 24000,
        });
        break;
      ...
      case 'server_tool_call':
        this.emit({
          ...
          status: String(parsed['status'] ?? 'started') as 'started' | 'completed' | 'failed',
        });
        break;
      case 'tool_call':
        this.emit({
